function [X_manpg, F_manpg,sparsity,time_manpg,iter,flag_succ,num_linesearch,mean_ssn,F,time_arr] = manpg_orth_sparse(B,option)
...
for iter = 2:maxiter
    ...
    if   F(iter -1)<=option.opt+1e-4 ||  normDsquared/t^2 < tol
    ...
    X = Z; AX = AZ;
    F(iter) = F_trial;
    time_arr(iter) = toc(tt);
    if iter == maxiter
        flag_maxiter =1;
    end
    
end
tol = max(max(abs(X)))/1e4;
X((abs(X)<=1e-4))=0;
X_manpg=X;
time_manpg = toc(tt);
...
%     time_manpg = 0;
    ...
    fprintf('\n ManPG:Iter ***  Fval *** CPU  **** sparsity ***inner_inexact&averge_No. ** opt_norm ** total_linsea \n');
    print_format = ' %i     %1.5e    %1.5f     %1.2f         %4i   %2.2f                %1.3e        %d \n';
