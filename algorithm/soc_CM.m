...
for itera=1:maxiter
    ...
    if itera>2
        normXQ = norm(X-Q,'fro');
        normQ = norm(Q,'fro');
        normX = norm(X,'fro');        normP = r;
        normXP = norm(X-P,'fro');
        AP = A*P;
        F_ad(itera)=sum(sum(X.*(AP)))+h(P);
        if  normXQ/max(1,max(normQ,normX)) + normXP/max(1,max(normP,normX)) <tol || F_ad(itera)<=option.F_mialm + 1e-4 
...
%     time_soc = 0;
    ...
    if norm(X_mialm*X_mialm'- X_Soc*X_Soc','fro')^2 > 0.1
