...
function [X_pamal, F_pamal,sparsity_pamal,time_PAMAL, error_XPQ,iter_pamal,flag_succ] =PAMAL_spca1(B,option)
...
for iterp = 1:maxiter
    ...
    if iterp>2
        ...
        if   normXQ/max(1,max(normQ,normX)) + normXP/max(1,max(normP,normX)) <tol
            ...
            if  F_PAMAL(iterp) <= option.F_mialm - 1e-2
