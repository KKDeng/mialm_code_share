function [X,F,sp,times,t,num_linesearch,num_inniter,flag] = Riemannian_mialm_CMS(Problem, X0, option)
...
problem.cost = @cost;
    ...
    function f = cost(X,BX)
        ...
        Prox = prox_g(temp,mu);
         if ~exist('BX', 'var')  
             f = cost_f(X) + cost_g(Prox) + 1/(2*mu) * sum(sum((temp -Prox ).^2));
         else
             f = cost_f(X,BX) + cost_g(Prox) + 1/(2*mu) * sum(sum((temp -Prox ).^2));
         end
    end
...
fprintf(1,print_format, t,F,times, sp,  num_inniter, info_err(t),info(end).gradnorm);
